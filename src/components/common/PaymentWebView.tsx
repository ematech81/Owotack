...
export function PaymentWebView({ visible, paymentLink, txRef, onSuccess, onCancel }: Props) {
  ...
  const handleRedirect = (url: string) => {
    ...
    try {
      ...
      // Korapay uses "reference"; Flutterwave used "tx_ref" — support both for safety
      const ref = params.get("reference") || params.get("tx_ref") || txRef;
      // Korapay uses "success"; Flutterwave used "successful" — support both
      if ((status === "success" || status === "successful") && ref) onSuccess(ref);
      else onCancel();
    } catch {
      onSuccess(txRef);
    }
