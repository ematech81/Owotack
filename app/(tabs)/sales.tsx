...
function ManualItemCard({
  item, index, total, isCollapsed, onUpdate, onRemove, onToggleCollapse,
}: {
  ...
  const displayName = item.productName?.trim() || `Item ${index + 1}`;
  ...
  return (
    <View style={[itemCardS.card, { zIndex: total - index }]}>
      ...
      <TouchableOpacity onPress={onToggleCollapse} activeOpacity={0.75}>
        <LinearGradient
        ...
        >
          <View style={itemCardS.cardHeaderLeft}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={itemCardS.cardTitle} numberOfLines={1}>{displayName}</Text>
              {/* Amount shown only when collapsed */}
              {isCollapsed && itemTotal > 0 && (
                <Text style={itemCardS.collapsedAmount}>{formatNaira(itemTotal)}</Text>
              )}
            </View>
          </View>

          <View style={itemCardS.cardHeaderRight}>
            <Animated.View style={{ transform: [{ rotate: chevronRotate }] }}>
              <Ionicons name="chevron-down" size={16} color={colors.textMuted} />
            </Animated.View>
            {total > 1 && (
              <TouchableOpacity
                style={itemCardS.removeBtn}
                onPress={onRemove}
...
const itemCardS = StyleSheet.create({
  ...
  collapsedAmount: {
    fontSize: 12, fontWeight: "700", color: colors.primary, marginTop: 2,
  },
  removeBtn: {
...
export default function AddSaleScreen() {
  ...
  const [collapsedItems, setCollapsedItems] = useState<boolean[]>([false]);
  ...
  useFocusEffect(useCallback(() => {
    ...
    draftStorage.load<SalesDraft>(draftKey).then((stored) => {
      ...
      if (data.mode === "manual") {
        ...
        setCollapsedItems(new Array(items.length).fill(false)); // all expanded on draft load
            ...
            <>
              ...
              {manualItems.map((item, index) => (
                <ManualItemCard
                  ...
                  isCollapsed={collapsedItems[index] ?? false}
                  onUpdate={(field, value) => updateManualItem(index, field, value)}
                  onRemove={() => {
                    setManualItems((p) => p.filter((_, i) => i !== index));
                    setCollapsedItems((p) => p.filter((_, i) => i !== index));
                  }}
                  onToggleCollapse={() =>
                    setCollapsedItems((p) => p.map((c, i) => (i === index ? !c : c)))
                  }
                />
              ))}
              ...
              {/* Add item button — collapses the last card before adding a new one */}
              <TouchableOpacity
                style={[screenS.addItemBtn, { borderColor: colors.primary, backgroundColor: colors.primary + "08" }]}
                onPress={() => {
                  setCollapsedItems((p) => {
                    const updated = [...p];
                    if (updated.length > 0) updated[updated.length - 1] = true;
                    return [...updated, false];
                  });
                  setManualItems((p) => [...p, emptyItem()]);
                }}
              >
