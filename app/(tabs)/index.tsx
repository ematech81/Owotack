...
import { creditService, ICredit } from "../../src/services/creditService";
...
export default function DashboardScreen() {
  ...
  const [overdueCredits, setOverdueCredits] = useState<ICredit[]>([]);
  ...
  const topCredit = overdueCredits[0] ?? null;
  const moreCount = Math.max(0, overdueCredits.length - 1);
      ...
      if (isOnline) {
        ...
        Promise.all([
          api.get<ApiResponse<Sale[]>>("/sales", {
            params: { startDate: `${start}T00:00:00.000Z`, endDate: `${end}T23:59:59.999Z`, limit: 100 },
          }).then(async (r) => {
            await salesDb.upsertFromServer(uid, r.data.data);
            const { startYMD, endYMD } = getDateRange(p);
            setPeriodSales(await salesDb.getByDateRange(uid, startYMD, endYMD));
          }).catch(() => {}),
          api.get<ApiResponse<Expense[]>>("/expenses", {
            ...
            const { startYMD, endYMD } = getDateRange(p);
            setPeriodExpenses(await expenseDb.getByDateRange(uid, startYMD, endYMD));
          }).catch(() => {}),
        ]);
      }
    },
  ...
  useEffect(() => {
    ...
    AsyncStorage.getItem(key).then((seen) => { if (!seen) setGuideVisible(true); });
  }, [user?._id]);
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      loadNotifications();
      creditService.list("overdue").then(setOverdueCredits).catch(() => {});
  ...
  const handlePeriod = (p: Period) => {
    if (!canAccessPeriod(p, reportsAccess)) { setUpgradeVisible(true); return; }
    setPeriod(p);
  };
      ...
      >
        ...
        <View style={styles.header}>
          <View style={styles.headerLeft}>
            <LinearGradient colors={[colors.primary, "#2D6A4F"]} style={styles.avatar}>
              <Text style={styles.avatarText}>{initials}</Text>
            </LinearGradient>
            <View>
              <Text style={styles.greetingText}>{greeting},</Text>
              <Text style={styles.nameText}>{user?.name?.split(" ")[0] ?? "Trader"} 👋</Text>
            </View>
          </View>
          <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
            ...
            <TouchableOpacity style={styles.notifBtn} activeOpacity={0.75} onPress={() => setNotifPanelVisible(true)}>
        ...
        {/* ── Payment Alert — real overdue credit data ── */}
        {topCredit && (
          <TouchableOpacity
          ...
          >
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.alertSub} numberOfLines={1}>
                {topCredit.customerName} owes{" "}
                <Text style={{ fontWeight: "800" }}>{formatNaira(topCredit.balance)}</Text>
        ...
        <View style={styles.ledgerHeader}>
          ...
          <TouchableOpacity style={styles.seeAllBtn} onPress={() => router.push("/(tabs)/ledger")}>
          ...
          <View style={styles.ledgerList}>
                  ...
                  <View style={styles.ledgerMetaRow}>
                    <View
                      style={[
                        ...
                        { backgroundColor: sale.syncStatus === "pending" ? "#F59E0B" : "#10B981" },
                      ]}
                    />
            ...
            {activeExpenses.slice(0, 2).map((exp) => (
              <View key={exp.localId || exp._id} style={styles.ledgerRow}>
                ...
                <View style={styles.ledgerInfo}>
                  <Text style={styles.ledgerName} numberOfLines={1}>{exp.description}</Text>
              ...
              <ScrollView showsVerticalScrollIndicator={false} style={{ maxHeight: 400 }}>
                {notifications.map((n) => (
                  ...
                  >
                    <View
                      style={[
                        styles.notifDotIndicator,
                        { backgroundColor: n.isRead ? "transparent" : colors.primary },
                      ]}
                    />
                    <View style={{ flex: 1 }}>
...
const makeStyles = (colors: ReturnType<typeof useTheme>) =>
  StyleSheet.create({
    ...
    notifOverlay: {
      ...
      justifyContent: "flex-start", paddingTop: 90, paddingHorizontal: 16,
    },
    notifPanel: {
    ...
    notifDotIndicator: { width: 8, height: 8, borderRadius: 4, marginTop: 5, flexShrink: 0 },
    ...
    // ── Hero Card — matches Ledger SummaryCard exactly ──
    heroCard: {
      borderRadius: 28,
      padding: 20,
      marginBottom: 28,
    ...
    alertCard: {
      ...
      ...Platform.select({
        ios: { shadowColor: "#DC2626", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 6 },
        android: { elevation: 2 },
      }),
    },
    alertIconCircle: {
