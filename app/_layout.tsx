...
export default function RootLayout() {
  ...
  useEffect(() => {
    ...
    const handleDeepLink = (url: string) => {
      try {
        ...
        // Validate ref matches the exact 8-char alphanumeric format generated by generateReferralCode()
        const validRef = ref && /^[A-Z0-9]{8}$/.test(ref) ? ref : null;
        if (parsed.pathname === "/join" && validRef && !useAuthStore.getState().isAuthenticated) {
          router.replace({ pathname: "/(auth)/phone", params: { ref: validRef } });
        }
      } catch {}
    };
  ...
  useEffect(() => {
    ...
    if (isAuthenticated) {
      const allowedOutsideTabs = ["subscribe", "subscription", "export", "receipt"];
    ...
    if (hasEverLoggedIn) {
      ...
      // Also allow forgot-PIN screens AND the full registration flow
      // so returning users can tap "Register Here" to create a second account.
      const allowedScreens = ["login", "forgot-pin", "otp", "reset-pin", "phone", "profile-setup", "set-pin"];
